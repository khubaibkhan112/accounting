...
class IncomeStatementExport implements FromArray, WithHeadings, WithStyles, WithColumnWidths
{
    protected $data;
    protected $summary;
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        // Add net income
        if (isset($this->summary['net_income'])) {
            $netIncomeRow = $totalRow + 2;
            $sheet->setCellValue('B' . $netIncomeRow, 'Net Income:');
            $sheet->setCellValue('C' . $netIncomeRow, $this->summary['net_income']);
            $sheet->getStyle('B' . $netIncomeRow . ':C' . $netIncomeRow)->applyFromArray([
                'font' => ['bold' => true, 'size' => 12],
            ]);
            $sheet->getStyle('C' . $netIncomeRow)->getNumberFormat()->setFormatCode('#,##0.00');
        }
    }
}
